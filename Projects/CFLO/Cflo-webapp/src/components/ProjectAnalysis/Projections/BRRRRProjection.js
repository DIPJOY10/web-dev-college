...
const useStyles = makeStyles((theme) => ({
  ...
  mainCont: {
    ...
    [theme.breakpoints.down('xs')]: {
      padding: `5px ${theme.sideMargin.sxScreen} 20px`,
    },
  },
  actionCont: {
    padding: "10px",
...
export default function BRRRRProjection({
  currentReport,
  resultData,
  expenseData,
  setView,
  ...
  const onTypeSelect = (value) => {
    ...
    let itr = 0;
    if (currentReport?.refinanceLoanTerm) {
      itr = Number(currentReport?.refinanceLoanTerm);
    }
    if (itr === 0) {
      itr = 30;
    }
  ...
  useEffect(() => {
    setCashFlowDataKeys(["Operating Income", "Operating Expense", "Cash Flow"]);
    setEquityDataKeys(["Property Value", "Loan Balance", "Total Equity"]);
      ...
      <div style={pageHight ? { height: pageHight, width: "100%", overflow: "hidden", margin: "0px", padding: "0px" } : { margin: "0px", padding: "0px" }} >
        {isViewOnly ? (
          <div className={classes.selectorCont} >
            ...
            <div>
              <MyPopOver
                ...
                appearContent={
                  <CustomBtn
                    startPart={isSmall ? null : <FilterListIcon />}
                    text={isSmall ? <FilterListIcon /> : selectedType}
                    style={isSmall ? { marginRight: "15px", padding: "6px 0px 0px" } : { marginRight: "15px" }}
                  />
                }
          ...
          <AnalysisHeader
            ...
            pageDesc={<>These projections show how this property will perform as a rental, after refinance, in the future.{` `}<a style={{ textDecoration: 'none', color: "#2196F3" }} href={configObject?.tutorialLinks?.brrrrLink} target="_blank" >View tutorial</a></>}
            rightBtns={<>
              <CustomBtn
        ...
        <ProjectionTable
          ...
          tablePrefix={{
            ...
            // "Post Tax Cash Flow": "" + "\xa0\xa0\xa0" + "$", // see line number 360
          }}
          tableHead={"CASH FLOW"}
          tableData={cashFlow}
          tableSuffix={""}
