...
import { offerCalculatorCalculation } from "./OfferCalculatorCalculation";
...
export default function OfferCalculator(props) {
    const history = useHistory();
    const classes = useStyles();
    const { teamId } = useParams();
    const {
        currentReport, setCurrentReport, reportId, setIsLoading,
        projectData, setProjectData, setLoadingText
    } = props;
    ...
    useDebounce(
        ...
        1200,
        [purchasePriceLimit, cashNeededLimit, totalCashInvestedLimit,
            role70Limit, priceSqFtLimit, cashFlowLimit, cOCReturnLimit,
            ROELimit, ROILimit, annualizedROILimit, rentToValueLimit,
    ...
    const updateOnClick = async (data) => {
        setLoadingText("Calculating Offer Price")
        setIsLoading(true)
        await updatePurchaseCriteria({
            criteriaId: projectData?.purchasePolicy?.[currentReport?.reportType]?._id,
            updateField: data
    ...
    useEffect(() => {
        if (currentReport && projectData) {
            let offerAm = offerCalculatorCalculation(currentReport, projectData)
            setOfferAmount(offerAm)
        }
    }, [currentReport, projectData])

    function numberWithCommas(x) {
        return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    }

    let OfferViewComponent = null;

    switch (offerAmount) {
        case -1:
            OfferViewComponent = <>
                <p className={classes.offerPriceHead} >Oops! Something is not right.</p>
                <p className={classes.offerPrice} >Please check the criteria.</p>
            </>
            break;

        case -2:
            OfferViewComponent = <p className={classes.offerPriceHead} >Oops! Something is not right.</p>
            OfferViewComponent = <p className={classes.offerPrice} >Please select at least one criteria</p>
            break;

        default:
            OfferViewComponent = <>
                <Tooltip title="click to select as purchase price" placement="top">
                    <p onClick={async () => { await updateReport({ purchasePrice: offerAmount }) }}
                    >$ {numberWithCommas(offerAmount)}</p>
                </Tooltip>
            </>
    }
    ...
    return (
        <div style={{ paddingTop: "50px" }} >
            <div className={classes.offerAndPriceCont} >
                <Paper className={classes.priceCont} >
                    <h3>Current Price</h3>
                    <div style={{ width: "100%", height: "10px" }} ></div>
                    <p className={classes.valuesSty} >$ {numberWithCommas(currentReport?.purchasePrice)}</p>
                </Paper>
                <Paper className={classes.priceCont} >
                    <h3>Highest Offer</h3>
                    <div style={{ width: "100%", height: "10px" }} >
                        {calculateBool && (<LinearProgress />)}
                    </div>
                    <p className={classes.valuesSty} >{OfferViewComponent}</p>
                </Paper>
            </div>
            ...
            <Paper className={classes.paperCont} >
                ...
                {currentReport?.reportType !== "Flip" && currentReport?.isFinancing && (
                    <div className={classes.inputMainCont} style={{ borderTop: "1px solid e9e8e8" }} >
                        <div className={classes.leftHalf} >
