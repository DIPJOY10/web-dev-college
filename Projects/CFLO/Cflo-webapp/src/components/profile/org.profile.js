...
const OrgProfile = (props)=>{
  ...
  const {orgDictionary} = useSelector((state)=>state.profile);
  const {organizationIds} = useSelector((state)=>state.auth);
  const [ownProfile, setOwnProfile] = useState(false);
  const {conversationDictionary, userConversation, selectedPersonId} = useSelector((state)=>state.chat);
  const orgMin = orgDictionary[orgId];
  const [org, setOrg] = useState(orgMin);


  const isAdmin = ()=>{
    const ownProfile = (organizationIds.indexOf(orgId))?true:false;
  };


  const {displayName, displayPicture} = org;
  let messagePath;
  const conversations = userConversation[orgId];
