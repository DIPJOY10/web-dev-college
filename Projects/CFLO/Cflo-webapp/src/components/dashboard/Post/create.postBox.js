...
function UploadPostBoxMemo({ post: postReceived, handleClose }) {
  const classes = useStyles();
  const history = useHistory();
  const dispatch = useDispatch();
  const { auth, file } = useSelector((state) => state);
...
const UploadPostBox = React.memo(UploadPostBoxMemo);
