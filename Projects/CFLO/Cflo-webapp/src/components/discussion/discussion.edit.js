...
function EditDiscussion(props) {
  ...
  const discussionOld = discussionDictionary[discussionId];
  const [discussion, setDiscussion] = useState(discussionOld);

  const {sortedProjectTeamIds, discussionLabels, teamDictionary} = useSelector((state)=>state.team);
  const teamId = discussion.team;
  const [description, setDescription] = useState(discussion.description);
  const [title, setTitle] = useState(discussion.title);
  const [startDate, setStartDate] = useState(''+discussion?.dates?.pS);
  const [endDate, setEndDate] = useState(''+ discussion?.dates?.pF);
  ...
  // console.log(startDate,endDate, discussion.dates,' discussions');
  ...
  const updateDiscussionApi = ()=>{
    Api.post('discussion/update', discussionObject).then((res)=>{
      if (res) {
        setDiscussion(res);

        setDiscussions([res], discussionReducer, dispatch);
        history.goBack();
      }
    })
        .catch((err)=>{
  ...
  return (
    <div className={classes.root}>
      ...
      <DescriptionInput
        ...
        placeholder={'Discussion Description (Optional)'}
