...
export default function PropCatCategories(props) {
  ...
  const {jobCats, jobCatDictionary} = dashboard;

  const {tagIds, setTagIds} = props;
  const [updated, setUpdated] = useState(false);
  const [state, setstate] = useState({});
  const mixedUsedCat = jobCats.filter((primaryCat) => primaryCat.main.name === 'Mixed Use');
  const normalJobCats = jobCats.filter((primaryCat) => primaryCat.main.name !== 'Mixed Use');
  ...
  return (
    <div className={classes.root}>
      ...
      <div>
        {normalJobCats.map((jobCat, i) => {
          const subs = jobCat.subs;
          return (
            <Accordion key={i}>
              <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="panel1a-content" id="panel1a-header">
                <Typography className={classes.heading}>{jobCat.main.name}</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <div>
                  {subs.map((subPropCat) => {
                    ...
                    return (
                      <Chip
                        ...
                        className={chip}
                        onClick={() => {
                          toggleTags(subPropCat, jobCat.main);
                        }}
                      />
                    );
                  })}
