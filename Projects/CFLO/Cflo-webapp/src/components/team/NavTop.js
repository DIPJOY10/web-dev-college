...
export default function TeamNavigation(props) {
  ...
  const {teamDictionary, sortedProjectTeamIds, sortedProjectMap} = teamReducer;

  const history = useHistory();
  const team = teamDictionary[teamId];
  const ancestors = team?.ancestors?.length > 0 ? team.ancestors : [teamId];
  const topTeamId = _.intersection(sortedProjectTeamIds, ancestors);
  ...
  return (
    ...
    >
      {open ? (
        <>
          <NavTree teamId={topTeamId}>{sortedProjectMap[topTeamId]}</NavTree>
        </>
      ) : null}
    </List>
  );
