...
const columns = [
    ...
    { id: 'numApps', label: 'Applications', align: 'center' },
...
//         rows.pop();
// }
function createData(avatar_view, displayName, title, numApps, subject, publishedAt, status_logo, status, edit_view) {
    ...
    return { avatar_view, displayName, title, numApps, subject, publishedAt, status_logo, status, edit_view };
}
...
export default function JobTable(props) {
    ...
    const getDataFromReducer = () => {
        var tempRows = [];
        for (const values of Object.values(jobDictionary)) {

            console.log(values, "Dictionary")
            avatar_view = values?.owner?.parent;
            // for (let i = 0; i < FilterStatus.length; i++) {
            // if (values?.status === FilterStatus[i]) {
            tempRows.push(createData(avatar_view, values?.owner?.parent?.displayName, values?.title, values?.numApps, values?.subject?.parent?.displayName, moment(values?.publishedAt).format('DD MMM YYYY'), status_logo, values?.status, (values?.status === "Active" ? <IconButton onClick={() => { history.push(`/dashboard/job/manage/${values?._id}`) }}><img src={manageIcon} /></IconButton> : <IconButton onClick={() => { history.push(`/dashboard/edit/job/${values?._id}`) }}><img src={createLogo} /></IconButton>)));
            // }
    ...
    useEffect(() => {
        getDataFromReducer()
    }, [jobDictionary, FilterStatus])


    // useEffect(() => {
    //     console.log(jobDictionary, "JobDict");
    // }, [jobDictionary])
    // }, [jobDictionary, FilterStatus])
    return (
        <Paper className={classes.root}>
            ...
            <TablePagination
                ...
                component="div"
                count={Object.keys(jobDictionary)?.length}
