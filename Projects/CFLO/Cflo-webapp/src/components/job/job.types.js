...
export default function JobCategories(props) {
  ...
  const { selectedJobCats, jobCats, jobDictionary, jobCatDictionary } = dashboard;

  const { jobId } = useParams();
  const job = jobDictionary[jobId];
  const { cats, setCats } = props;
  const [tagIds, settagIds] = useState(selectedJobCats);
  ...
  return (
    <div className={classes.root}>
      <Paper component="ul" className={classes.selectedPaper}>
        <Typography className={classes.heading}>Selected Job Categories : </Typography>
        {tagIds.map((tagId) => {
