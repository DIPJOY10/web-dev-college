...
export default function MultipleChoiceGridResponse(props) {
  ...
  const oldFormResId = oldFormRes[0]?._id
  ...
  const handleChange = (event) => {
    ...
    const oldIndex = resOptionIds.indexOf(row)
    ...
    if (oldIndex !== -1) {
      ...
      const newObj = addItem(row, col, newRemObj.options, newRemObj.optionCols)
      ...
      </div>
      {
        optionIds.map((optionId, index) => {
          ...
          return (<div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', }}>
            <RadioGroup aria-label="gender" value={oldColId} name="gender1" onChange={handleChange} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: "space-evenly", width: "100%" }} >
              {
                (optionColIds).map((optionId, index) => {
                  const option = optionColDict[optionId];
                  return (
                    <FormControlLabel value={option?._id} name={optionRow?._id} control={<Radio color="primary" />} label={""} classes={{ root: classes.formControlLabel }} />
                  );
                })
              }
            </RadioGroup>
